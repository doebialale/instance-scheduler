#!/usr/bin/python3 /home/ec2-user/ManageCloudVMsss.py -a stop -p "aws:ca-central-1"
...
def manage_aws_hosts(operation, region):
    ...
    # instancese = ec2.instances.filter(Filters=[{'Name': 'instance-state-name', 'Values': ['pending', 'running', 'stopped',]},])    
    ...
    for reservation in response['Reservations']:
        for instance in reservation['Instances']:
            ...
            instance_tag = str(instance['Tags'])
            no_delete = (instance_tag.find("Do-Not-Delete"))

            # Skip instances that are terminated and instances with the 'Do-Not-Delete' tag value
            if instance_state != 'terminated' and no_delete == -1:
...
''', formatter_class=argparse.RawTextHelpFormatter)
    ...
    parser.add_argument('-p', '--platforms', dest='platforms', default='',
